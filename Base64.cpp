...
std::string Base64::encode(const std::vector<char>& datax)
{
...
std::vector<char> Base64::decode(const std::string& datax)
{
  std::string::size_type  i;
  char c;
  char c1;
  std::string::size_type  len = datax.length();
  std::vector<char>  ret;

  for (i = 0; i < len; ++i)
  {
    c = (char) cvt.find(datax[i]);
    ++i;
    c1 = (char) cvt.find(datax[i]);
    c = (c << 2) | ((c1 >> 4) & 0x3);
    ret.push_back(c);
    if (++i < len)
    {
