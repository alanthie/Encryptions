...
namespace generate_rsa
{
    ...
    struct rsa_key
    {
        ...
        typeuinteger get_d() { return cryptoAL::key_util::val(s_d);}


        typeuinteger encode(const std::string& s)
        {
            typeuinteger n = get_n();
            typeuinteger m = cryptoAL::key_util::val(s);
            typeuinteger r = cryptoAL::key_util::mod_pow(m, get_e(), n);
            return r;
        }
        ...
        std::string decode(const typeuinteger& v)
        {
            typeuinteger n = get_n();
            typeuinteger m = v;
            typeuinteger r = cryptoAL::key_util::mod_pow(m, get_d(), n);
            std::string  s = cryptoAL::key_util::to_base64(r);
            return s;
        }
    ...
    struct PRIVATE_KEY
    {
        ...
        void to_rsa_key(rsa_key& rkey, const typeuinteger& n, const typeuinteger& e, const typeuinteger& d, uint32_t keysize_in_bits)
        {
            rkey.key_size_in_bits = keysize_in_bits;
            {
                std::stringstream ss;
                ss << n ; // base 10
                rkey.s_n = cryptoAL::key_util::base10_to_base64(ss.str());
            }

            {
                std::stringstream ss;
                ss << e ;
                rkey.s_e = cryptoAL::key_util::base10_to_base64(ss.str());
            }

            {
                std::stringstream ss;
                ss << d ;
                rkey.s_d = cryptoAL::key_util::base10_to_base64(ss.str());
            }
/*
            std::cout << "-----------------------------" << std::endl;
